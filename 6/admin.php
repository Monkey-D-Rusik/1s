...
function print_table_line($values,$columns){
  print'<tr>';
  $PK=$columns[0];
  foreach ($columns as $col) {
    print'<td>'.$values[$col].'</td>';
  }
  print '<form action="" method="POST"><input name="PK" value="'.$values[$PK].'" hidden><td><input type="submit" name="change" value="изменить"></td>';
  print '<td class="delete"><input type="submit" name="delete" value="удалить"></td></form>';
  print'<tr>';
}
function print_table_head($values){
  print'<tr>';
...
//если метод POST
  ...
  if(!empty($_POST['powers_counter']) && (!empty($_POST['powers_counter_value']) || !empty($_COOKIE['powers_counter_value']))){
